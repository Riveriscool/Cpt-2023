...
def delete_json_item(username, token):
...
def updateMessageBoard(token, username, message):
        print("starting updatemessageboard!!")
        shouldDelete = False
        with open('userdata.json', "r") as f:
            data = json.load(f)
        ...
        if shouldDelete:
            ...
            with open("messageboard.txt", "w") as f:
                f.write("MESSAGES:")
                f.close()
        print('deleted')
...
while True:
    ...
    if choice[0] == "l":
        ...
        delete_json_item(choice[1],choice[2])
        clientConnected.send("S".encode())
