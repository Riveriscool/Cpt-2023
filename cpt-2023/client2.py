...
def SendMsg():
    ...
    with open("usercorrelations1.txt", "r") as f:
        username = f.read()
    with open("clientsidedata1.txt", "r") as f:
        token = f.read()
...
def homepage():
    ...
    MessageEntry = CTkTextbox(master=Homepage, width=500, corner_radius=30,border_width=5, border_color="Royal Blue")
    MessageEntry.insert("0.0", "Your message here!")
    MessageEntry.grid(row=6,column=3)
    SendMessage = CTkButton(master=Homepage, text="Send!",height=20,width=10, corner_radius=30,command=SendMsg)
    SendMessage.grid(row=6, column=4)
...
def commence():
    ...
    if msg1 != None:
        ...
        print(data)
        with open("clientsidedata.txt1", "w") as f:
            f.write(data)
...
def submitLogin():
    ...
    with open("usercorrelations1.txt", "w") as f:
